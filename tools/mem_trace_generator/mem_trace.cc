...
void set_mem_op(trace_info_cpu_s *mem_op, uint64_t inst_addr, uint64_t addr1,
                bool is_write) {
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2)
    printf("Usage: %s <dram-read-file>. Defalt is generating stream memory
  addresses \n"); if (argc >= 3) { fname = fname =(argv[1]); fname << argv[1];
  ...
  */
  ...
  for (int threadid = 0; threadid < threadnum; threadid++) {
    ...
    trace_info_cpu_s *mem_trace =
        (trace_info_cpu_s *)malloc(sizeof(trace_info_cpu_s));
    trace_info_cpu_s *fence_trace =
        (trace_info_cpu_s *)malloc(sizeof(trace_info_cpu_s));
...
#define INDEP_TRACE
          ...
          set_mem_op(mem_trace, (++inst_addr) * 2, t_addr,
      ...
      case 1:
        ...
        if (myfile.is_open()) {
          while (getline(myfile, line)) {
            ...
            if (req_type == 0)
              set_mem_op(mem_trace, (++inst_addr) * 2, t_addr,
                         req_type);  // 0: read operation 1: write operation //
            else
              set_mem_op(mem_trace, (++inst_addr) * 2 + 1, t_addr,
                         req_type);  // 0: read operation 1: write operation //
            gzwrite(trace_output, mem_trace, sizeof(trace_info_cpu_s));
          }
          myfile.close();
