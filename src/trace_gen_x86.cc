...
enum CPU_OPCODE_enum {
  ...
  PREFETCH_T2,
  GPU_EN,
  CPU_MEM_EXT_OP,  // To denote that the op is a continuation of the same parent
                   // op but has more mem accesses.
  CPU_OPCODE_LAST,
} CPU_OPCODE;
  ...
  if (xed_error == XED_ERROR_NONE) {
    ...
    for (uint32_t ii = 0; ii < memops; ++ii) {
      ...
      if (xed_decoded_inst_mem_read(&xedd, ii) ||
        ...
        // TODO: LEA instructions reach here. LEA is typically not a memory
        // read, but in some cases (immediate operand) its behaviour can be
        // similar to MOV. Not sure if LEA with imm reaches here.
      }
    }
...
void InstHandler_x86::processMem(uint64_t v, uint64_t p, uint8_t s, int w) {
  ...
  if ((prev_op->m_num_ld) && !w) {
    if (!prev_op->m_ld_vaddr1) {
      ...
      // addr per ins. Certain instructions can have multiple loads and stores.
      // Skip logging those right now unless the user explicitly enables them.
      // Push a new child ins with the same PC address and print it later to the
      // trace.
    ...
    // For example, some JMP or CALL instructions which trigger state
    // save/restore.

    // Note: The trace structure has room for only 2 ld addresses and 1 st addr
    // per ins. Certain instructions can have multiple loads and stores. Skip
    // logging those right now unless the user explicitly enables them.
