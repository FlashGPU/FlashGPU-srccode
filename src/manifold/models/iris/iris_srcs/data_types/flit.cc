...
string Flit::toString() const {
  stringstream str;
  str << " type: " << type << " vc: " << virtual_channel
      << " no_phits: " << no_phits << " pkt_length: " << pkt_length <<
#ifdef _DEBUG
      " flit_id: " << flit_id <<
#endif
      " ";
...
string HeadFlit::toString() const {
  stringstream str;
  str << " srcn: " << src_node << " dstn: " << dst_node
      << " dst_compid: " << dst_component_id << " addr: " << hex << address
      << dec << " class: " << mclass << " ";
  str << static_cast<const Flit *>(this)->toString();
  return str.str();
} /* -----  end of method HeadFlit::toString  ----- */

void HeadFlit::populate_head_flit(void) {
...
void BodyFlit::populate_body_flit(void) {
  /* May want to construct control bit streams etc here */
}
...
void TailFlit::populate_tail_flit(void) {
  /* May want to construct control bit streams etc here */
}
...
void FlitLevelPacket::add(Flit *f) {
  switch (f->type) {
    case HEAD: {
...
Flit *FlitLevelPacket::get_next_flit(void) {
  Flit *np = flits.front();
  flits.pop_front();
  return np;
}

uint FlitLevelPacket::size(void) {
  return flits.size();
} /* -----  end of method FlitLevelPacket::size  ----- */

bool FlitLevelPacket::valid_packet(void) {
  if (flits.size())
