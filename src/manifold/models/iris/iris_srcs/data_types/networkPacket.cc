...
bool NetworkPacket::operator==(const NetworkPacket &p) {
...
string NetworkPacket::toString() const {
  stringstream str;
  str << " src_node: " << hex << (uint)this->src_node << dec
      << " dst_node: " << (uint)this->dst_node
      << " dst_compId: " << (uint)this->dst_component_id << " address: " << hex
      << (uint)this->address << dec << " pkt_length: " << this->pkt_length
      << " payload_length: " << this->payload_length;
  return str.str();
}

int NetworkPacket::Serialize(NetworkPacket p, unsigned char *buf) {
  return 0;
}

NetworkPacket NetworkPacket::Deserialize(const unsigned char *data, int len) {
  NetworkPacket p;
  /*
    cout << "\nDSER LEN: " << len;
...
void NetworkPacket::from_flit_level_packet(FlitLevelPacket *flp) {
...
void NetworkPacket::to_flit_level_packet(FlitLevelPacket *flp, uint lw) {
