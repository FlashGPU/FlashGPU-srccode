...
void TwoNode::parse_config(std::map<std::string, std::string> &p) {
  /* Defaults */
  grid_size = 2;
  no_nodes = 2;
...
std::string TwoNode::print_stats() {
...
void TwoNode::connect_interface_terminal() {
  /* Connect INTERFACE -> TERMINAL*/
  for (uint i = 0; i < no_terminals; i++) {
    terminals.at(i)->ni =
        static_cast<manifold::kernel::Component *>(interfaces.at(i));
    interfaces.at(i)->terminal =
        static_cast<manifold::kernel::Component *>(terminals.at(i));
...
void TwoNode::assign_node_ids(component_type t) {
  switch (t) {
    case IRIS_TERMINAL: {
      for (uint i = 0; i < no_nodes; i++)
...
void TwoNode::connect_interface_routers() {
  int LATENCY = 1;
  /*  Connect for the output links of the router */
  for (uint i = 0; i < no_terminals; i++) {
    /*  Interface DATA */
    manifold::kernel::Manifold::Connect(
        interface_ids.at(i), 0, router_ids.at(i), 4,
    ...
    /*  Interface SIGNAL */
    manifold::kernel::Manifold::Connect(
        interface_ids.at(i), 1, router_ids.at(i), 6,
        &IrisRouter::handle_link_arrival,
        static_cast<manifold::kernel::Ticks_t>(LATENCY));
    manifold::kernel::Manifold::Connect(
        router_ids.at(i), 2, interface_ids.at(i), 3,
        &IrisInterface::handle_link_arrival,
        static_cast<manifold::kernel::Ticks_t>(LATENCY));
  }
...
void TwoNode::set_router_outports(uint ind) {
...
void TwoNode::connect_routers() {
  int LATENCY = 1;
  /* Router->Router DATA */
  manifold::kernel::Manifold::Connect(
      router_ids.at(0), 1, router_ids.at(1), 5,
      &IrisRouter::handle_link_arrival,
      static_cast<manifold::kernel::Ticks_t>(LATENCY));
  manifold::kernel::Manifold::Connect(
      router_ids.at(1), 1, router_ids.at(0), 5,
      &IrisRouter::handle_link_arrival,
      static_cast<manifold::kernel::Ticks_t>(LATENCY));

  /* Router->Router SIGNAL */
  manifold::kernel::Manifold::Connect(
      router_ids.at(0), 3, router_ids.at(1), 7,
      &IrisRouter::handle_link_arrival,
      static_cast<manifold::kernel::Ticks_t>(LATENCY));
  manifold::kernel::Manifold::Connect(
      router_ids.at(1), 3, router_ids.at(0), 7,
      &IrisRouter::handle_link_arrival,
      static_cast<manifold::kernel::Ticks_t>(LATENCY));
