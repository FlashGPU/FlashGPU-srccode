...
void Mesh::parse_config(std::map<std::string, std::string> &p) {
  /* Defaults */
  no_nodes = 4;
  grid_size = 4;
  ports = 3;
...
std::string Mesh::print_stats(component_type ty) {
...
void Mesh::connect_interface_terminal() {
...
void Mesh::connect_interface_routers() {
  int LATENCY = 1;
...
void Mesh::set_router_outports(uint ind) {
...
void Mesh::assign_node_ids(component_type t) {
  switch (t) {
    case IRIS_TERMINAL: {
      for (uint i = 0; i < terminals.size(); i++)
