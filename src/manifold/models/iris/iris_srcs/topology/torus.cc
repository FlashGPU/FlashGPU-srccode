...
void Torus::parse_config(std::map<std::string, std::string> &p) {
  /* Defaults */
  no_nodes = 3;
...
std::string Torus::print_stats(component_type ty) {
...
void Torus::connect_interface_terminal() {
...
void Torus::connect_interface_routers() {
  int LATENCY = 1;
...
void Torus::set_router_outports(uint ind) {
...
void Torus::connect_routers() {
  ...
  for (uint i = 0; i < grid_size; i++) {
    ...
    printf("\n Connect %d p%d -> %d p%d", rno, ports + 1, end_rno,
           3 * ports + 2);

    printf("\n Connect %d p%d -> %d p%d", end_rno, 2, rno, 2 * ports + 1);
    printf("\n Connect %d p%d -> %d p%d", end_rno, ports + 2, rno,
           3 * ports + 1);
#endif
    ...
    // going west <-
    manifold::kernel::Manifold::Connect(
        rno, 1, (end_rno), 2 * ports + 2, &IrisRouter::handle_link_arrival,
        static_cast<manifold::kernel::Ticks_t>(LATENCY));

    manifold::kernel::Manifold::Connect(
        (rno), ports + 1, (end_rno), 3 * ports + 2,
        &IrisRouter::handle_link_arrival,
        static_cast<manifold::kernel::Ticks_t>(LATENCY));

    // going east ->
    manifold::kernel::Manifold::Connect(
        end_rno, 2, rno, 2 * ports + 1, &IrisRouter::handle_link_arrival,
        static_cast<manifold::kernel::Ticks_t>(LATENCY));
    manifold::kernel::Manifold::Connect(
        end_rno, ports + 2, rno, 3 * ports + 1,
        &IrisRouter::handle_link_arrival,
        static_cast<manifold::kernel::Ticks_t>(LATENCY));
  }
  ...
  for (uint i = 0; i < grid_size; i++) {
    ...
    printf("\n Connect %d p%d -> %d p%d", rno, ports + 3, end_rno,
           3 * ports + 4);

    printf("\n Connect %d p%d -> %d p%d", end_rno, 4, rno, 2 * ports + 3);
    printf("\n Connect %d p%d -> %d p%d", end_rno, ports + 4, rno,
           3 * ports + 3);
#endif
    ...
    // going west <-
    manifold::kernel::Manifold::Connect(
        (rno), 3, (end_rno), 2 * ports + 4, &IrisRouter::handle_link_arrival,
        static_cast<manifold::kernel::Ticks_t>(LATENCY));

    manifold::kernel::Manifold::Connect(
        (rno), ports + 3, (end_rno), 3 * ports + 4,
        &IrisRouter::handle_link_arrival,
        static_cast<manifold::kernel::Ticks_t>(LATENCY));

    // going east ->
    manifold::kernel::Manifold::Connect(
        end_rno, 4, rno, 2 * ports + 3, &IrisRouter::handle_link_arrival,
        static_cast<manifold::kernel::Ticks_t>(LATENCY));
    manifold::kernel::Manifold::Connect(
        end_rno, ports + 4, rno, 3 * ports + 3,
        &IrisRouter::handle_link_arrival,
        static_cast<manifold::kernel::Ticks_t>(LATENCY));
  }
...
void Torus::assign_node_ids(component_type t) {
  switch (t) {
    case IRIS_TERMINAL: {
      for (uint i = 0; i < terminals.size(); i++)
