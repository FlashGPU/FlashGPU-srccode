...
void Ring::parse_config(std::map<std::string, std::string> &p) {
  /* Defaults */
  no_nodes = 3;
...
void Ring::connect_interface_terminal()  // std::vector <ManifoldProcessor*>
                                         // &g_macsim_terminals)
{
...
void Ring::connect_interface_routers() {
  int LATENCY = 1;
  ...
  for (uint i = 0; i < no_nodes; i++) {
    ...
    // routers.at(i)->node_id = interfaces.at(i)->node_id; //handled in
    // init_network assert ( routers.at(i)->node_id == i );
  }

  return;
}

void Ring::set_router_outports(uint ind) {
...
void Ring::connect_routers() {
  ...
  for (uint i = 1; i < no_nodes; i++) {
    ...
    manifold::kernel::Manifold::Connect(
        router_ids.at(i), 1, router_ids.at(i - 1), 2 * ports + 2,
        &IrisRouter::handle_link_arrival,
        static_cast<manifold::kernel::Ticks_t>(LATENCY));

    /* Router->Router SIGNAL */
    manifold::kernel::Manifold::Connect(
        router_ids.at(i), ports + 1, router_ids.at(i - 1), 3 * ports + 2,
        &IrisRouter::handle_link_arrival,
        static_cast<manifold::kernel::Ticks_t>(LATENCY));
    ...
    /* Router->Router DATA */
    manifold::kernel::Manifold::Connect(
        router_ids.at(i - 1), 2, router_ids.at(i), 2 * ports + 1,
        &IrisRouter::handle_link_arrival,
        static_cast<manifold::kernel::Ticks_t>(LATENCY));

    /* Router->Router SIGNAL */
    manifold::kernel::Manifold::Connect(
        router_ids.at(i - 1), ports + 2, router_ids.at(i), 3 * ports + 1,
        &IrisRouter::handle_link_arrival,
        static_cast<manifold::kernel::Ticks_t>(LATENCY));
  }
  ...
  // going west <-
  manifold::kernel::Manifold::Connect(
      router_ids.at(0), 1, router_ids.at(no_nodes - 1), 2 * ports + 2,
      &IrisRouter::handle_link_arrival,
      static_cast<manifold::kernel::Ticks_t>(LATENCY));

  manifold::kernel::Manifold::Connect(
      router_ids.at(0), ports + 1, router_ids.at(no_nodes - 1), 3 * ports + 2,
      &IrisRouter::handle_link_arrival,
      static_cast<manifold::kernel::Ticks_t>(LATENCY));

  // going east ->
  manifold::kernel::Manifold::Connect(
      router_ids.at(no_nodes - 1), 2, router_ids.at(0), 2 * ports + 1,
      &IrisRouter::handle_link_arrival,
      static_cast<manifold::kernel::Ticks_t>(LATENCY));
  manifold::kernel::Manifold::Connect(
      router_ids.at(no_nodes - 1), ports + 2, router_ids.at(0), 3 * ports + 1,
      &IrisRouter::handle_link_arrival,
      static_cast<manifold::kernel::Ticks_t>(LATENCY));
...
void Ring::assign_node_ids(component_type t) {
  switch (t) {
    case IRIS_TERMINAL: {
      for (uint i = 0; i < no_nodes; i++)
