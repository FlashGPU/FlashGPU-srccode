...
class GenericVcAllocator {
 public:
  ...
  bool request(uint out_port, uint out_vc, uint in_port,
               uint in_vc); /* \fn Request output port allocation.
 \param in_port: Requesting in_port
 \param in_vc: Requesting in_vc.*/
  bool is_empty(void);      /* \fn Checks if the vca unit has requests */
  bool is_empty(uint i); /* \fn Checks if the vca unit has requests for the ith
                            row ( i=out_port*ports+out_vc) */
  void pick_winner(std::vector<GenericBuffer> &in_buffers);
  void clear_winner(uint op, uint ovc, uint ip,
                    uint ivc); /* \fn Clear winner */
