...
void GenericSwitchArbiter::resize(uint p, uint v) {
  ports = p;
  vcs = v;
...
bool GenericSwitchArbiter::is_requested(uint oport, uint inport, uint och) {
  if (oport >= ports || inport >= ports || och >= vcs) {
    std::cout << " Error in SWA oport: " << oport << " inp: " << inport
              << " och: " << och << std::endl;
    exit(1);
  }
...
void GenericSwitchArbiter::request(uint oport, uint ovc, uint inport,
                                   uint ivc) {
  ...
  requesting_inputs[oport][inport * vcs + ovc].in_time =
      manifold::kernel::Manifold::NowTicks();
  return;
}

SA_unit GenericSwitchArbiter::pick_winner(uint oport) {
  return do_round_robin_arbitration(oport);
}

SA_unit GenericSwitchArbiter::do_round_robin_arbitration(uint oport) {
...
void GenericSwitchArbiter::clear_requestor(uint oport, uint inport, uint och) {
  requested[oport][inport * vcs + och] = false;
  return;
}

bool GenericSwitchArbiter::is_empty() {
  for (uint i = 0; i < ports; i++)
    for (uint j = 0; j < (ports * vcs); j++)
      if (requested[i][j])
        return false;
...
std::string GenericSwitchArbiter::toString() const {
