...
void SimpleMC::parse_config(std::map<std::string, std::string> &p) {
  ni_buffer_width = 2;
  no_nodes = 1;
...
void SimpleMC::handle_new_packet_event(int p, NetworkPacket *data) {
  proc_recv = false;
  ...
  if (mc_response_buffer.size() < max_mc_buffer_size) {
    ...
    if (curr_pkt) {
      //    _DBG("************ MC got new packet ************** %d size:%d",
      //    curr_pkt->proc_buffer_id, mc_response_buffer.size());
      stat_packets_in++;
...
&IrisTerminal::handle_new_packet_event, static_cast<IrisTerminal*>(this),
(int)SEND_DATA, temp);
       }
     */
  }
...
void SimpleMC::handle_issue_pkt_event(int port) {
  ...
  if (mc_response_buffer.size() && is_buffer_empty) {
    ...
    if (np && manifold::kernel::Manifold::NowTicks() > np->sent_time &&
        (manifold::kernel::Manifold::NowTicks() - np->sent_time) >=
            memory_latency) {
      //        NetworkPacket* np = mc_response_buffer.front();
      ...
      //            _DBG(" ******* MC sending new pkt  %d to:%d respbuffsize:%d
      //            buffready %lld", send_buffer_id, np->dst_node,
      //            mc_response_buffer.size(),stat_packets_out );
      ...
      if (!proc_recv) {
        ...
        proc_recv = true;
        manifold::kernel::Manifold::Schedule(
            1, &IrisTerminal::handle_new_packet_event,
...
void SimpleMC::handle_send_credit_event(int port, uint64_t data) {
...
void SimpleMC::handle_update_credit_event(int p, uint64_t data) {
  assert(data < ni_buffer_width);
  ni_buffers[data] = true;
  return;
}

void SimpleMC::tick(void) {
  return;
}

void SimpleMC::tock(void) {
  handle_issue_pkt_event(SEND_DATA);
  return;
}

std::string SimpleMC::print_stats() const {
