...
void GenericVcAllocator::setup(uint p, uint v) {
  ports = p;
  vcs = v;
...
bool GenericVcAllocator::request(uint op, uint ovc, uint ip, uint invc) {
...
bool GenericVcAllocator::is_empty() {
  for (uint i = 0; i < ports; i++)
    if (!is_empty(i))
      return false;
...
int GenericVcAllocator::no_requestors(uint op) {
  uint no = 0;
  for (uint j = 0; j < ports * vcs; j++)
    if (requestors[op][j])
      no++;
...
void GenericVcAllocator::pick_winner(std::vector<GenericBuffer> &in_buffers) {
  if (!is_empty()) {
    for (uint i = 0; i < ports; i++)
      for (uint j = 0; j < vcs; j++) {
        ...
        if (it != ovc_tokens[i].end()) {
          ...
          for (uint jj = st_loc; jj < ports * vcs; jj++)
            if (requestors[i][jj] && requestors_vc[i][jj] == j) {
              ...
              if( f->type == HEAD )
              {
                  cout << "vca winner:" << ((HeadFlit*)f)->req->m_id <<  ",src:"
             << node_ip << ",dst:" << ((HeadFlit*)f)->dst_node <<  endl;
              }
             // */
              break;
            }
