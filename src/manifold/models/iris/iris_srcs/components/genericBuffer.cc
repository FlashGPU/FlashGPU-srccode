 ...
 * ===  FUNCTION
 * ====================================================================== Name:
 * peek Description:  Uses the pull channel. Make sure to set it before peek is
...
uint GenericBuffer::get_occupancy(uint ch) const {
  return buffers[ch].size();
} /* -----  end of function GenericBuffer::get_occupancy  ----- */

void GenericBuffer::resize(uint v, uint bs) {
  vcs = v;
  buffer_size = bs;
  buffers.resize(vcs);
...
bool GenericBuffer::is_channel_full(uint ch) const {
  /* this is the buffer size that the router is configured for the
   * implementation allow for a bigger buffer[i].size which is the simulation
   * artifact and not the buffer size in the physical router */
...
std::string GenericBuffer::toString() const {
