...
void SimpleRouter::parse_config(map<string, string> &p) {
  ports = 3;
  vcs = 4;
  credits = 4;
...
void SimpleRouter::reset_stats(void) {
  // stats init
  stat_avg_buff = 0;
  stat_packets_out = 0;
  stat_packets_in = 0;
...
void SimpleRouter::handle_link_arrival(int port, LinkData *data) {
  switch (data->type) {
    case FLIT: {
...
#endif
      if (data->f->type == HEAD) {
        ((HeadFlit *)data->f)->req->m_noc_cycle =
            manifold::kernel::Manifold::NowTicks();
#if 1
        ...
        if (inport == 0) {
          ...
          if (nodeLatency.find(m_id) == nodeLatency.end())
            ...
            nodeLatency[m_id] =
                manifold::kernel::Manifold::NowTicks() + nodeLatency[m_id];
        }
        ...
        cout << manifold::kernel::Manifold::NowTicks() << ",p,"
             ...
             << endl;
        /*
                   cout << manifold::kernel::Manifold::NowTicks() << ",b," <<
           node_id << ","; for(uint i=0; i<ports; i++) for(uint j=0; j<vcs; j++)
        ...
        // for debugging only!
        int stage = -1;
        static const int stage_state[3][3] = {
            // transposed array for column major order
        ...
        stage +=
            (stage == 1 && ((HeadFlit *)data->f)->req->m_state == 12) ? 4 : 0;

        cout << manifold::kernel::Manifold::NowTicks() << ", "
             << ((HeadFlit *)data->f)->req->m_id << ", " << stage << ", "
             << node_id << ", " << ((HeadFlit *)data->f)->dst_node << ", "
             << mem_state_copy[((HeadFlit *)data->f)->req->m_state] << ", \n";
#endif
    ...
    case CREDIT: {
      ...
      {
        cout << manifold::kernel::Manifold::NowTicks()
             << ", node:4 credit received"
             << downstream_credits[inport][data->vc] << endl;
      }
      break;
    }
...
void SimpleRouter::do_input_buffering(HeadFlit *hf, uint inport, uint invc) {
  ...
  input_buffer_state[inport * vcs + invc].pkt_arrival_time =
      manifold::kernel::Manifold::NowTicks();
...
void SimpleRouter::do_vc_allocation() {
  ...
  for (uint i = 0; i < ports; i++)
    for (uint ai = 0; ai < vca.current_winners[i].size(); ai++) {
      ...
      if (input_buffer_state[msgid].pipe_stage ==
          VCA_REQUESTED)  // && downstream_credits[op][oc]==credits)
      {
      ...
      if (in_buffers[ip].get_occupancy(ic) > 0 &&
        ...
        if (f->type == TAIL || f->pkt_length == 1) {
          ...
          if (op != 5 && op != 0)
            ...
            stat_3d_packets_out++;
          //                    printf("Time%lld node:%d stat_3d_packets_out
          //                    %lld stat_2d_packets_out:%lld op%d addr:%llx\n",
          //                           manifold::kernel::Manifold::NowTicks(),
          //                           node_id,
          //                           stat_3d_packets_out,stat_2d_packets_out,op,
          //                           input_buffer_state[i].address);
        ...
        Send(data_outports.at(op), ld);  // schedule cannot be used here as the
                                         // component is not on the same LP
        ...
        if (input_buffer_state[i].sa_head_done) {
          ...
          if (sa_winner.port == ip &&
            ...
            /*	        Flit* f = in_buffers[ip].buffers[ic].front();
                            ...
                            {
                                //cout << "swa winner:" <<
               ((HeadFlit*)f)->req->m_id <<  ",src:" << node_id << ",dst:" <<
               ((HeadFlit*)f)->dst_node <<  endl;
                            }
                            for(int i=0; i<ports; i++)
                            {
                                ...
                                {
                                    for (auto I =
               in_buffers[i].buffers[j].begin(), E =
               in_buffers[i].buffers[j].end(); I != E; ++I) { if( (*I) != NULL
               && (*I)->type == HEAD )
                                            {
                                                HeadFlit* H = (HeadFlit*)(*I);
                                                cout << "node:" << node_id <<
               ",m_id:" << H->req->m_id << "," << H->req->m_noc_cycle << endl;
                                            }
                                    }
                                }
                            }
...
void SimpleRouter::tick(void) {
  return;
}

extern void dump_state_at_deadlock(macsim_c *simBase);
void SimpleRouter::tock(void) {
  /*
   ...
   *
   ...
   && input_buffer_state[i].pkt_arrival_time+1000 <
   manifold::kernel::Manifold::NowTicks())
   {
   fprintf(stderr,"\n\nDeadlock at Router %d node %d Msg id %d Fid%d",
   GetComponentId(), node_id, i, input_buffer_state[i].fid);
...
std::string SimpleRouter::print_stats(void) const {
  ...
  str << "\n SimpleRouter[" << node_id << "] packets_in: " << stat_packets_in
      ...
      << "\n SimpleRouter[" << node_id << "] avg_router_latency: "
      << (avg_router_latency + 0.0) / stat_packets_out << "\n SimpleRouter["
      << node_id << "] last_pkt_out_cy: "
      << stat_last_flit_out_cycle
      //        << "\n SimpleRouter[" << node_id << "] stat_3d_packets_out: " <<
      //        stat_3d_packets_out
      //        << "\n SimpleRouter[" << node_id << "] stat_2d_packets_out: " <<
      //        stat_2d_packets_out
      //        << "\n SimpleRouter[" << node_id << "] stat_3d_flits_out: " <<
      //        stat_3d_flits_out
      //        << "\n SimpleRouter[" << node_id << "] stat_2d_flits_out: " <<
      //        stat_2d_flits_out
      //        << "\n SimpleRouter[" << node_id << "] inj_nodes_packets_out: "
      //        << stat_packets_out - ( stat_2d_packets_out +
      //        stat_3d_packets_out )
      //        << "\n SimpleRouter[" << node_id << "] local_remote_ratio: " <<
      //        stat_3d_packets_out*1.0/stat_2d_packets_out
      //        << "\n SimpleRouter[" << node_id << "] stat_3d_flits_out: " <<
      //        stat_3d_flits_out
      //        << "\n SimpleRouter[" << node_id << "] stat_2d_flits_out: " <<
      //        stat_2d_flits_out
...
std::string SimpleRouter::print_csv_stats(void) const {
  std::stringstream str;
  str << node_id << "," << stat_packets_out << "," << stat_flits_in << ","
      << stat_flits_out << "," << ib_cycles << "," << sa_cycles << ","
      << vca_cycles << "," << st_cycles << ",";
  ...
  for (uint i = 0; i < ports; i++)
    for (uint j = 0; j < vcs; j++)
      buff +=
          stat_pp_avg_buff[i][j] * 1.0 / manifold::kernel::Manifold::NowTicks();
  buff /= ports * vcs;
  ...
  str << buff << "," << (avg_router_latency + 0.0) / stat_packets_out << ","
      << lat;
...
void SimpleRouter::power_stats() {
  ...
  // link power
  Pdynamic = 0.5 * activity_factor *
             LinkDynamicEnergyPerBitPerMeter(link_len, Vdd) * freq * link_len *
             (double)data_width;
  Pleakage = LinkLeakagePowerPerMeter(link_len, Vdd) * link_len * data_width;
  Plink = (Pdynamic + Pleakage) * PARM(in_port);

  double Eavg = SIM_router_stat_energy(info, router, 0, path, AVG_ENERGY,
                                       activity_factor, 0, freq);
  cout << "Node:" << node_id << " Packets: " << stat_packets_in
       << " Router Power:" << Eavg * freq << " Link Power:" << Plink << "\n";
}
