...
bool SimpleArbiter::is_requested(uint ch) {
  //    assert( ch < requests.size() );
  return requests[ch];
}

bool SimpleArbiter::is_empty(void) {
  for (uint i = 0; i < no_channels; i++)
    if (requests[i])
      return false;
...
void SimpleArbiter::request(uint ch) {
  requests[ch] = true;
  return;
}

uint SimpleArbiter::pick_winner(void) {
  ...
  std::cout << " ERROR: Interface Arbiter was either empty or called and dint "
               "have a winner"
            << std::endl;
  exit(1);
}
...
void NInterface::parse_config(std::map<std::string, std::string> &p) {
  no_vcs = 4;
  credits = 1;
  link_width = 128;
...
void NInterface::handle_link_arrival(int port, LinkData *data) {
  switch (data->type) {
    case FLIT: {
#ifdef _DEBUG
      _DBG(" Int got flit vc%d ft%d fid:%d", data->vc, data->f->type,
           data->f->flit_id);
#endif
      if (data->f->type == HEAD)
        stat_packets_in++;
      ...
      if (data->f->type != TAIL && data->f->pkt_length != 1) {
        ...
        // manifold::kernel::Manifold::Schedule(1,
        // &IrisRouter::handle_link_arrival, static_cast<IrisRouter*>(router),
        // (uint)SEND_DATA, ld );
      }
      break;
    }
...
void NInterface::handle_new_packet_event(int port, NetworkPacket *data) {
  // std::cout << "IRIS New packet at " << node_id << " procBufferID " <<
  // data->proc_buffer_id << " src: " << data->src_node << " dst: " <<
  // data->dst_node << "\n";
  /*  Convert the packet to flits and store in empty buffer */
  //    assert ( data->proc_buffer_id < no_vcs);
  data->to_flit_level_packet(&proc_out_buffer[data->proc_buffer_id],
                             link_width);
...
void NInterface::handle_issue_pkt_event(int port, uint64_t data) {
  return;
}

void NInterface::handle_send_credit_event(int port, uint64_t data) {
  /* Send ack to terminal
   * */
  manifold::kernel::Manifold::Schedule(
      1, &ManifoldProcessor::handle_update_credit_event,
      static_cast<ManifoldProcessor *>(terminal), (int)SEND_SIG, data);
  return;
}

void NInterface::handle_update_credit_event(int data) {
  if (data > no_vcs) {
...
void NInterface::tock(void) {
  ...
  for (uint i = 0; i < no_vcs; i++)
    if (downstream_credits[i] > 0 && router_out_buffer.get_occupancy(i) > 0 &&
      ...
      if ((f->type != HEAD) ||
          (f->type == HEAD && downstream_credits[i] == credits)) {
  ...
  if (!arbiter.is_empty()) {
    ...
    if (f->type == TAIL || f->pkt_length == 1) {
      if (f->type == TAIL)
        static_cast<TailFlit *>(f)->enter_network_time =
...
#ifdef _DEBUG
    _DBG(" SEND FLIT %d Flit is %s", winner, f->toString().c_str());
    std::cout.flush();
#endif
    // Use manifold send here to push the ld obj out
    // manifold::kernel::Manifold::Schedule(1, &IrisRouter::handle_link_arrival,
    // static_cast<IrisRouter*>(router), (uint)SEND_DATA, ld );
  ...
  if (!static_cast<ManifoldProcessor *>(terminal)->proc_recv) {
    ...
    if (found) {
      ...
      manifold::kernel::Manifold::Schedule(
          1, &ManifoldProcessor::handle_new_packet_event,
          static_cast<ManifoldProcessor *>(terminal), (uint)SEND_DATA, np);
  ...
  for (uint i = 0; i < no_vcs; i++) {
    if (router_in_buffer.get_occupancy(i) > 0 &&
        (proc_ib_flit_index.at(i) < (proc_in_buffer.at(i)).pkt_length ||
         proc_ib_flit_index.at(i) == 0)) {
