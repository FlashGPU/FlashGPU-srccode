...
uint GenericRC::route_x_y(uint dest) {
  uint oport = -1;
...
// want spinal routers to be flexible: prefer y traversal, but can use x if
// adjacent router is also spinal
// FIXME only does X then Y for now, as GPU nodes only have lateral links -
// consider using MECS eventually?
void GenericRC::route_x_y_hetero(HeadFlit *hf)
...
void GenericRC::route_torus(HeadFlit *hf) {
...
void GenericRC::route_ring_uni(HeadFlit *hf) {
  grid_size = no_nodes;
  uint myx = node_id;
...
void GenericRC::route_ring(HeadFlit *hf) {
  grid_size = no_nodes;
  uint myx = node_id;
  uint destx = hf->dst_node;
...
void GenericRC::route_twonode(HeadFlit *hf) {
  if (node_id == 0) {
    if (hf->dst_node == 1)
      possible_out_ports.push_back(1);
    else
...
uint GenericRC::get_output_port(uint ch) {
  uint oport = -1;
  if (addresses[ch].last_adaptive_port ==
      addresses[ch].possible_out_ports.size())
    addresses[ch].last_adaptive_port = 0;
...
std::string GenericRC::toString() const {
