...
void dump_configuration(std::map<std::string, std::string> &p) {
  std::cerr << " Knob\tValue\n";
...
int main(int argc, char *argv[]) {
  ...
  if (Mytid == TERMINAL_PROC) {
    for (uint i = 0; i < no_nodes; i++) {
      IrisTerminal *terminal =
          manifold::kernel::Component::GetComponent<IrisTerminal>(
              tp->terminal_ids.at(i));
      IrisInterface *interface =
          manifold::kernel::Component::GetComponent<IrisInterface>(
              tp->interface_ids.at(i));
      ...
      manifold::kernel::Clock::Register<IrisInterface>(
          interface, &IrisInterface::tick, &IrisInterface::tock);
    ...
    for (uint i = 0; i < no_nodes; i++) {
      ...
      std::cerr << " Terminal " << i << " id : " << tp->terminal_ids.at(i)
                << " Nid: " << terminal->node_id << " "
                << typeid(terminal).name() << std::endl;
  ...
  if (Mytid == ROUTER_PROC) {
    for (uint i = 0; i < no_nodes; i++) {
      IrisRouter *rr = manifold::kernel::Component::GetComponent<IrisRouter>(
          tp->router_ids.at(i));
      assert(rr != NULL);
      manifold::kernel::Clock::Register<IrisRouter>(rr, &IrisRouter::tick,
                                                    &IrisRouter::tock);
