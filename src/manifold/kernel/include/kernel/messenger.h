...
class Messenger {
 public:
  ...
  void send_uint32_msg(int dest, int compIndex, int inputIndex, double sendTime,
                       double recvTime, uint32_t data);

  void send_uint64_msg(int dest, int compIndex, int inputIndex,
                       Ticks_t sendTick, Ticks_t recvTick, uint64_t data);

  void send_uint64_msg(int dest, int compIndex, int inputIndex, double sendTime,
                       double recvTime, uint64_t data);

  void send_serial_msg(int dest, int compIndex, int inputIndex,
                       Ticks_t sendTick, Ticks_t recvTick, unsigned char *data,
                       int len);

  void send_serial_msg(int dest, int compIndex, int inputIndex, double sendTime,
                       double recvTime, unsigned char *data, int len);
 ...
 private:
  ...
  Message_s
      m_msg;  // message holder; avoid allocating/deallocating all the time
