...
#ifndef NO_MPI
//! Add an input for the component. An input is only created when the
//! corresponding output component is in a different LP. We store the component
//! and the handler so that we can handle input data.
...
void Component::AddInput(int inputIndex, void (T::*handler)(int, T2), T *obj,
                         Clock *c, bool isTimed = false, bool isHalf = false) {
  // First insure the vector is large enough, add nulls if not
  while ((int)inLinks.size() <= inputIndex)
    inLinks.push_back(0);

  assert(inLinks[inputIndex] ==
         0);  // input for the index cannot have already been created.
  inLinks[inputIndex] =
      new LinkInput<T2, T>(handler, obj, inputIndex, c, isTimed, isHalf);
}
#endif

// Implementations for the Create functions
template <typename T>
CompId_t Component::Create(LpId_t lp, CompName_t name) {
...
CompId_t Component::Create(LpId_t lp, const T1 &t1, const T2 &t2,
                           CompName_t name) {
  if (lp == Manifold::GetRank()) {
    T *n = new T(t1, t2);
    allComponents.push_back(ComponentLpMapping(n, lp));
...
CompId_t Component::Create(LpId_t lp, const T1 &t1, const T2 &t2, const T3 &t3,
                           CompName_t name) {
  if (lp == Manifold::GetRank()) {
    T *n = new T(t1, t2, t3);
    allComponents.push_back(ComponentLpMapping(n, lp));
...
{
  if (allComponents[id].component == 0)
    return 0;  // Does not exist on this lp
...
T *Component::GetComponent(CompName_t name)  // Get component by id
{
  ...
  if (name == "None" || iter == allNames.end())
    return 0;
  if (allComponents[allNames[name]].component == 0)
    return 0;  // Does not exist on this lp
