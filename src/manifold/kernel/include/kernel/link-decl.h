...
class LinkOutputBase {
 public:
   ...
   */
  LinkOutputBase(Ticks_t lat, int ii, Clock *c, Time_t delay, bool isTimed,
                 bool isHalf)
      : latency(lat),
        inputIndex(ii),
        clock(c),
...
class LinkOutput : public LinkOutputBase<T1> {
 public:
   ...
   *  @arg \c f The 2 param callback function ptr used upon a link arrival
   * event.
   ...
   */
  LinkOutput(Link<T1> *link, void (OBJ::*f)(int, T1), OBJ *o, Ticks_t latency,
             int inputIndex, Clock *c, Time_t d, bool isT, bool isH)
...
class LinkOutputRemote : public LinkOutputBase<T> {
 public:
   ...
   *  @arg \c isH True if the latency is in half ticks.
   */
  LinkOutputRemote(Link<T> *link, int compIdx, int inputIndex, Clock *c,
                   Ticks_t latency, Time_t d, bool isT, bool isH);
...
class LinkInputBase {
 public:
  ...
  virtual void Recv(Ticks_t tick, Time_t time, unsigned char *data,
                    int len) = 0;
...
// pointer type, such as MyType*. In this case, calling T :: Deserialize()
// directly in Recv() below would cause a compile problem.
template <typename T>
 ...
 *  necessary is that templated functions can not be virtual. In Recv()
 * eventually we want to call ScheduleRxEvent() and somehow pass the templated
 * data. If ScheduleRxEvent() is tempalted, then it cannot be virtual and be
 * overridden by LinkInput.
 */
template <typename T>
class LinkInputBaseT : public LinkInputBase {
 public:
  ...
  void Recv(Ticks_t tick, Time_t time, unsigned char *data, int len) {
    // Cannot call T :: Deserialize() directly, because if T is a pointer type
    // such as MyType*, then compiler would complain Deserialize() is not a
    // member of MyType*. Therefore, we created 2 template functions above to
    // solve this problem.
...
/** A LinkInput is created when the sending component is in a different LP. It
 * stores some info so that an input data can be processed. The info includes
 * pointer to Clock, as well as the data type. The data type is stored by making
 * LinkInput a template class.
 */
...
class LinkInput : public LinkInputBaseT<T> {
 public:
  ...
  LinkInput(void (OBJ::*f)(int, T), OBJ *o, int inputIndex, Clock *c,
      ...
      : LinkInputBaseT<T>(inputIndex, c, isTimed, isHalf), obj(o), handler(f) {}
...
class Link : public LinkBase {
 public:
   ...
   */
  template <typename O>
  void AddOutput(void (O::*f)(int, T), O *o, Ticks_t l, int ii, Clock *c,
                 Time_t delay = 0.0, bool isTimed = false, bool isHalf = false);

#ifndef NO_MPI
  /** Adds an output to this Link, the other component is in a different LP.
   *  @arg \c compIdx Index of the other component.
   *  @arg \c inputIndex Input index(port) number on the receiving component to
   * use.
   *  @arg \c c Reference clock used for latency in ticks.
   *  @arg \c l Latency in ticks of this output.
   *  @arg \c delay Latency of the link in terms of time.
   ...
   *  @arg \c isHalf True if the latency in ticks is acutally half ticks.
   */
  void AddOutputRemote(CompId_t compIdx, int inputIndex, Clock *c, Ticks_t l,
                       Time_t delay = 0.0, bool isTimed = false,
                       bool isHalf = false);
#endif  //#ifndef NO_MPI
