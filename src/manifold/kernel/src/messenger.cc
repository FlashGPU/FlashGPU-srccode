...
void Messenger ::allGather(char *item, int itemSize, char *recvbuf) {
  MPI_Allgather(item, itemSize, MPI_BYTE, recvbuf, itemSize, MPI_BYTE,
                MPI_COMM_WORLD);
}
...
void Messenger ::send_uint32_msg(int dest, int compIndex, int inputIndex,
                                 Ticks_t sendTick, Ticks_t recvTick,
                                 uint32_t data) {
  ...
  MPI_Pack(&msg_type, 1, MPI_UNSIGNED, buf, Buf_size, &position,
           MPI_COMM_WORLD);
  ...
  // Ticks_t == uint64_t
  MPI_Pack(&sendTick, 1, MPI_UNSIGNED_LONG_LONG, buf, Buf_size, &position,
           MPI_COMM_WORLD);
  MPI_Pack(&recvTick, 1, MPI_UNSIGNED_LONG_LONG, buf, Buf_size, &position,
           MPI_COMM_WORLD);
...
void Messenger ::send_uint64_msg(int dest, int compIndex, int inputIndex,
  ...
  MPI_Pack(&recvTick, 1, MPI_UNSIGNED_LONG_LONG, buf, Buf_size, &position,
           MPI_COMM_WORLD);
  MPI_Pack(&data, 1, MPI_UNSIGNED_LONG_LONG, buf, Buf_size, &position,
           MPI_COMM_WORLD);
...
void Messenger ::send_serial_msg(int dest, int compIndex, int inputIndex,
  ...
  MPI_Pack(&msg_type, 1, MPI_UNSIGNED, m_send_buf, m_send_buf_size, &position,
           MPI_COMM_WORLD);
  MPI_Pack(&compIndex, 1, MPI_INT, m_send_buf, m_send_buf_size, &position,
           MPI_COMM_WORLD);
  MPI_Pack(&inputIndex, 1, MPI_INT, m_send_buf, m_send_buf_size, &position,
           MPI_COMM_WORLD);
  MPI_Pack(&isTick, 1, MPI_INT, m_send_buf, m_send_buf_size, &position,
           MPI_COMM_WORLD);
  // Ticks_t == uint64_t
  MPI_Pack(&sendTick, 1, MPI_UNSIGNED_LONG_LONG, m_send_buf, m_send_buf_size,
           &position, MPI_COMM_WORLD);
  MPI_Pack(&recvTick, 1, MPI_UNSIGNED_LONG_LONG, m_send_buf, m_send_buf_size,
           &position, MPI_COMM_WORLD);
  MPI_Pack(&len, 1, MPI_INT, m_send_buf, m_send_buf_size, &position,
           MPI_COMM_WORLD);
  MPI_Pack(data, len, MPI_UNSIGNED_CHAR, m_send_buf, m_send_buf_size, &position,
           MPI_COMM_WORLD);
...
void Messenger ::send_serial_msg(int dest, int compIndex, int inputIndex,
  ...
  MPI_Pack(&isTick, 1, MPI_INT, m_send_buf, m_send_buf_size, &position,
           MPI_COMM_WORLD);
  MPI_Pack(&sendTime, 1, MPI_DOUBLE, m_send_buf, m_send_buf_size, &position,
           MPI_COMM_WORLD);
  MPI_Pack(&recvTime, 1, MPI_DOUBLE, m_send_buf, m_send_buf_size, &position,
           MPI_COMM_WORLD);
  MPI_Pack(&len, 1, MPI_INT, m_send_buf, m_send_buf_size, &position,
           MPI_COMM_WORLD);
  MPI_Pack(data, len, MPI_UNSIGNED_CHAR, m_send_buf, m_send_buf_size, &position,
...
void Messenger :: start_irecv()
{
    ...
    for(int i=0; i<m_nodeSize; i++) {
        MPI_Irecv(m_buf[i], MAX_MSG_SIZE, MPI_CHAR, i, MSG_TAG, MPI_COMM_WORLD,
&m_requests[i]);
    }
  ...
  /*
      ...
      *received = 0;
      ...
      if(*received != 0) {
          ...
          m_requests[index]=0;
          MPI_Irecv(m_buf[index], MAX_MSG_SIZE, MPI_CHAR, index, MSG_TAG,
     MPI_COMM_WORLD, &m_requests[index]); m_numReceived++;
      }
      else {
  ...
  *received = 0;
  ...
  if (flag != 0) {
    *received = 1;
    ...
    MPI_Recv(m_recv_buf, m_recv_buf_size, MPI_PACKED, status.MPI_SOURCE,
             MSG_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
Message_s &Messenger ::unpack_message(unsigned char *buf) {
  ...
  // MPI_Recv(buf, MAX_MSG_SIZE, MPI_PACKED, MPI_ANY_SOURCE, MSG_TAG,
  // MPI_COMM_WORLD, &status);

  position = 0;
  MPI_Unpack(buf, m_recv_buf_size, &position, &(m_msg.type), 1, MPI_UNSIGNED,
             MPI_COMM_WORLD);

  switch (m_msg.type) {
    case Message_s ::M_UINT32:
      MPI_Unpack(buf, m_recv_buf_size, &position, &(m_msg.compIndex), 1,
                 MPI_INT, MPI_COMM_WORLD);
      MPI_Unpack(buf, m_recv_buf_size, &position, &(m_msg.inputIndex), 1,
                 MPI_INT, MPI_COMM_WORLD);
      MPI_Unpack(buf, m_recv_buf_size, &position, &(m_msg.isTick), 1, MPI_INT,
                 MPI_COMM_WORLD);
      if (m_msg.isTick == 0) {
        MPI_Unpack(buf, m_recv_buf_size, &position, &(m_msg.sendTime), 1,
                   MPI_DOUBLE, MPI_COMM_WORLD);
        MPI_Unpack(buf, m_recv_buf_size, &position, &(m_msg.recvTime), 1,
                   MPI_DOUBLE, MPI_COMM_WORLD);
      }
      else {
      ...
      MPI_Unpack(buf, m_recv_buf_size, &position, &(m_msg.uint32_data), 1,
    ...
    case Message_s ::M_UINT64:
      ...
      MPI_Unpack(buf, m_recv_buf_size, &position, &(m_msg.uint64_data), 1,
                 MPI_UNSIGNED_LONG_LONG, MPI_COMM_WORLD);
      break;
    case Message_s ::M_SERIAL:
      ...
      MPI_Unpack(buf, m_recv_buf_size, &position, &(m_msg.data_len), 1, MPI_INT,
                 MPI_COMM_WORLD);
      m_msg.data = &m_recv_buf[position];
      // MPI_Unpack(buf, m_recv_buf_size, &position, &(m_msg.data),
      // m_msg.data_len, MPI_UNSIGNED_CHAR,
      //                                                        MPI_COMM_WORLD);
      break;
