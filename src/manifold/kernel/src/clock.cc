...
void Clock::Rising() {  // Call rising edge function on all registered objects
...
void Clock::Falling() {  // Call falling edge function on all registered objects
  list<tickObjBase *>::iterator iter;
  for (iter = tickObjs.begin(); iter != tickObjs.end(); iter++) {
    tickObjBase *to = *iter;
    if (to->enabled)
...
TickEventId Clock::Insert(TickEventBase *ev) {
  ...
  if (ev->time >= CLOCK_CALENDAR_LENGTH) {  // Just insert into the sorted queue
...
TickEventId Clock::InsertHalf(
    TickEventBase *ev) {  // Time is in  units of half-ticks.  If odd, toggle
                          // rising/falling
                          /*
                            Ticks_t when = nextTick + ev->time;
  ...
  if (!nextRising)
    thisTick++;  // thisTick is the number of half ticks
  Ticks_t when = thisTick + ev->time;
  if (ev->time & 0x01)
    ev->rising = !nextRising;
  else
    ev->rising = nextRising;

  // And convert time to RELATIVE (relative to now, i.e, nextTick) full tick
  // units
  ev->time = when / 2 - nextTick;
  return Insert(ev);
}

void Clock::Cancel(TickEventId) {  // Implement later
}

Time_t Clock::NextTickTime() const {
  Time_t time = (Time_t)nextTick / freq;
  if (!nextRising)
    time += period / 2.0;  // The falling is 1/2 period later
  return time;
}
