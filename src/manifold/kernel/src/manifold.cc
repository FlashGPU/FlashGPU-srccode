...
#endif
  double nextTime =
      nextClockTime;  // time of next clock/timed event, whichever is earlier
...
EventId
Manifold::Peek() {  // Return eventid for earliest event, but do not remove it
...
bool Manifold::isSafeToProcess(double requestTime) {
  ...
  if (requestTime <= grantedTime) {
    ...
    TheMessenger.allGather((char *)&(LBTS[nodeId]), sizeof(LBTS_Msg),
                           (char *)LBTS);
