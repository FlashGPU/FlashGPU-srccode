...
void LinkOutputRemote<uint32_t>::ScheduleRxEvent() {
  if (timed) {
    ...
    TheMessenger.send_uint32_msg(
        dest, compIndex, inputIndex, Manifold::NowHalfTicks(*(this->clock)),
        Manifold::NowHalfTicks(*(this->clock)) + latency, data);
  }
  else {
    TheMessenger.send_uint32_msg(
        dest, compIndex, inputIndex, Manifold::NowTicks(*(this->clock)),
