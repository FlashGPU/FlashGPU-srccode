...
class LinkTest : public CppUnit::TestFixture {
 ...
 public:
  ...
  void testSend_0() {
    ...
    link1->AddOutput<MyObj1>(&MyObj1::handler1, obj1, latency, inputIdx, &clock,
                             tLatency, isTimed, isHalf);
   ...
   */
  void testSend_1() {
    // This test case uses SchuduleClockHalf; we should test different
    // latencies.
    ...
    for (unsigned int i = 0; i < sizeof(lats) / sizeof(Ticks_t); i++) {
      ...
      link1->AddOutput<MyObj1>(&MyObj1::handler1, obj1, latency, inputIdx,
                               &clock, tLatency, isTimed, isHalf);
  ...
  void testSend_2() {
    ...
    // verify handler called at right time
    CPPUNIT_ASSERT_DOUBLES_EQUAL(scheduling + tLatency, obj1->getTimeCalled(),
                                 DOUBLE_COMP_DELTA);
    // verify handler called with right parameter
    CPPUNIT_ASSERT(d1 == obj1->getD1());
  ...
  /**
   * Test SendTick()
   * In this test case, the dynamically given tick latency is greater than the
   * default latency.
   */
  void testSendTick_0() {
    Link<MyDataType1> *link1 = new Link<MyDataType1>();
  ...
  /**
   * Test SendTick()
   * In this test case, the dynamically given tick latency is smaller than the
   * default latency.
   */
  void testSendTick_1() {
    Link<MyDataType1> *link1 = new Link<MyDataType1>();
  ...
  static CppUnit::Test *suite() {
    ...
    mySuite->addTest(
        new CppUnit::TestCaller<LinkTest>("testSend_0", &LinkTest::testSend_0));
    mySuite->addTest(
        new CppUnit::TestCaller<LinkTest>("testSend_1", &LinkTest::testSend_1));
    mySuite->addTest(
        new CppUnit::TestCaller<LinkTest>("testSend_2", &LinkTest::testSend_2));
    mySuite->addTest(
        new CppUnit::TestCaller<LinkTest>("testSend_3", &LinkTest::testSend_3));

    mySuite->addTest(new CppUnit::TestCaller<LinkTest>(
        "testSendTick_0", &LinkTest::testSendTick_0));
    mySuite->addTest(new CppUnit::TestCaller<LinkTest>(
        "testSendTick_1", &LinkTest::testSendTick_1));
