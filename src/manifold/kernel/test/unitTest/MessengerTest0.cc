...
class MessengerTest : public CppUnit::TestFixture {
 private:
  static const double DOUBLE_COMP_DELTA =
      1.0E-5;  //! Delta for comparing doubles.
 ...
 public:
  ...
  void test_send_uint32_msg_1() {
    ...
    if (Mytid == 0) {  // task 0 creates messages; sends them and recvs the echo
      ...
      double when[SIZE][2];  // first column = sendTime; 2nd column = recvTime
                             // recvTime > sendTime
      when[0][0] =
          random() / (RAND_MAX + 1.0) * 10;  // a number between 0 and 10
      ...
      for (int i = 0; i < SIZE; i++) {
        ...
        // send a message
        TheMessenger.send_uint32_msg(1, cidx[i], iidx[i], when[i][0],
                                     when[i][1], data[i]);
        ...
        do {
          ...
          if (received != 0) {
            ...
            CPPUNIT_ASSERT_DOUBLES_EQUAL(msg.sendTime, when[i][0],
                                         DOUBLE_COMP_DELTA);
            CPPUNIT_ASSERT_DOUBLES_EQUAL(msg.recvTime, when[i][1],
                                         DOUBLE_COMP_DELTA);
      ...
      while (num < SIZE) {
        ...
        do {
          ...
          if (received != 0) {
            ...
            // send it back
            TheMessenger.send_uint32_msg(0, msg.compIndex, msg.inputIndex,
                                         msg.sendTime, msg.recvTime,
                                         msg.uint32_data);
          }
        } while (received == 0);
        num++;
  ...
  void test_send_uint32_msg_2() {
    ...
    if (Mytid == 0) {  // task 0 creates messages; sends them and recvs the echo
      ...
      when[0][0] = (Ticks_t)(random() / (RAND_MAX + 1.0) *
                             10);  // a number between 0 and 10
      when[0][1] = when[0][0] + 1;
      for (int i = 1; i < SIZE; i++) {
        // when[i][0] = when[i-1][1] + d;   1 <= d < 6
        when[i][0] =
            when[i - 1][1] + (Ticks_t)(random() / (RAND_MAX + 1.0) * 5 + 1);
        when[i][1] =
            when[i][0] + (Ticks_t)(random() / (RAND_MAX + 1.0) * 5 + 1);
      }
  ...
  void test_send_uint64_msg_1() {
    ...
    if (Mytid == 0) {  // task 0 creates messages; sends them and recvs the echo
      ...
      for (int i = 0; i < SIZE; i++) {
        ...
        // send a message
        TheMessenger.send_uint64_msg(1, cidx[i], iidx[i], when[i][0],
                                     when[i][1], data[i]);
      ...
      while (num < SIZE) {
        ...
        do {
          ...
          if (received != 0) {
            ...
            // send it back
            TheMessenger.send_uint64_msg(0, msg.compIndex, msg.inputIndex,
                                         msg.sendTime, msg.recvTime,
                                         msg.uint64_data);
          }
        } while (received == 0);
  ...
  void test_send_serial_msg_1() {
    ...
    if (Mytid == 0) {  // task 0 creates messages; sends them and recvs the echo
      ...
      for (int i = 0; i < SIZE; i++) {
        ...
        // send a message
        TheMessenger.send_serial_msg(1, cidx[i], iidx[i], when[i][0],
                                     when[i][1], buffer, len);
        ...
        do {
          ...
          if (received != 0) {
            ...
            CPPUNIT_ASSERT(data[i] ==
                           MyPkt ::Deserialize(msg.data, msg.data_len));
          }
        } while (received == 0);
      }
    }
      ...
      while (num < SIZE) {
        ...
        do {
          ...
          if (received != 0) {
            ...
            // send it back
            TheMessenger.send_serial_msg(0, msg.compIndex, msg.inputIndex,
                                         msg.sendTime, msg.recvTime, msg.data,
                                         msg.data_len);
          }
        } while (received == 0);
        num++;
      ...
      while (num < SIZE) {
        ...
        do {
          ...
          if (received != 0) {
            ...
            // send it back
            TheMessenger.send_serial_msg(0, msg.compIndex, msg.inputIndex,
                                         msg.sendTick, msg.recvTick, msg.data,
                                         msg.data_len);
          }
        } while (received == 0);
        num++;
  ...
  static CppUnit::Test *suite() {
    ...
    mySuite->addTest(new CppUnit::TestCaller<MessengerTest>(
        "test_send_uint32_msg_1", &MessengerTest::test_send_uint32_msg_1));
    mySuite->addTest(new CppUnit::TestCaller<MessengerTest>(
        "test_send_uint32_msg_2", &MessengerTest::test_send_uint32_msg_2));
    mySuite->addTest(new CppUnit::TestCaller<MessengerTest>(
        "test_send_uint64_msg_1", &MessengerTest::test_send_uint64_msg_1));
    mySuite->addTest(new CppUnit::TestCaller<MessengerTest>(
        "test_send_uint64_msg_2", &MessengerTest::test_send_uint64_msg_2));
    mySuite->addTest(new CppUnit::TestCaller<MessengerTest>(
        "test_send_serial_msg_1", &MessengerTest::test_send_serial_msg_1));
    mySuite->addTest(new CppUnit::TestCaller<MessengerTest>(
        "test_send_serial_msg_2", &MessengerTest::test_send_serial_msg_2));
