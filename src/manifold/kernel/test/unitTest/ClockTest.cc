...
class MyObj1 {
 ...
 public:
  ...
  void risingTick() { m_risingTickCalled += 1; }

  void fallingTick() { m_fallingTickCalled += 1; }
};
...
class ClockTest : public CppUnit::TestFixture {
 ...
 public:
  ...
  void testRising_0() {
    ...
    tickObjBase *tobj1 = Clock::Register<MyObj1>(comp1, &MyObj1::risingTick,
                                                 &MyObj1::fallingTick);
    tickObjBase *tobj2 = Clock::Register<MyObj1>(comp2, &MyObj1::risingTick,
                                                 &MyObj1::fallingTick);
  ...
  void testRising_1() {
    ...
    // register with master clock
    tickObjBase *tobj1 = Clock::Register<MyObj1>(
        Clock1, comp1, &MyObj1::risingTick, &MyObj1::fallingTick);
    tickObjBase *tobj2 = Clock::Register<MyObj1>(
        Clock1, comp2, &MyObj1::risingTick, &MyObj1::fallingTick);
  ...
  void testInsert_0() {
    ...
    TickEvent0<MyObj1, MyObj1> *ev0 = new TickEvent0<MyObj1, MyObj1>(
        WHEN, MasterClock, &MyObj1::risingTick, comp1);
    MasterClock.Insert(ev0);

    // cast away const because we need to remove the event later.
    Clock::EventVec_t &v =
        const_cast<Clock::EventVec_t &>(MasterClock.getCalendar()[WHEN]);
  ...
  void testInsert_1() {
    ...
    // cast away const because we need to remove the event later.
    std::multimap<Ticks_t, TickEventBase *> &mmm =
        const_cast<std::multimap<Ticks_t, TickEventBase *> &>(
            MasterClock.getEventMap());

    std::pair<std::multimap<Ticks_t, TickEventBase *>::iterator,
              std::multimap<Ticks_t, TickEventBase *>::iterator>
  ...
  static CppUnit::Test *suite() {
    ...
    mySuite->addTest(new CppUnit::TestCaller<ClockTest>(
        "testRising_0", &ClockTest::testRising_0));
    mySuite->addTest(new CppUnit::TestCaller<ClockTest>(
        "testRising_1", &ClockTest::testRising_1));
    mySuite->addTest(new CppUnit::TestCaller<ClockTest>(
        "testFalling_0", &ClockTest::testFalling_0));
    mySuite->addTest(new CppUnit::TestCaller<ClockTest>(
        "testFalling_1", &ClockTest::testFalling_1));

    mySuite->addTest(new CppUnit::TestCaller<ClockTest>(
        "testInsert_0", &ClockTest::testInsert_0));
    mySuite->addTest(new CppUnit::TestCaller<ClockTest>(
        "testInsert_1", &ClockTest::testInsert_1));
    return mySuite;
  }
};
