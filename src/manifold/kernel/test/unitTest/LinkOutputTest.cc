...
class LinkOutputTest : public CppUnit::TestFixture {
 ...
 public:
  ...
  void testSend_0() {
    ...
    LinkOutput<MyDataType1, MyObj1> *linkOutput1 =
        new LinkOutput<MyDataType1, MyObj1>(link1, &MyObj1::handler1, obj1,
                                            latency, inputIdx, &clock, tLatency,
                                            isTimed, isHalf);

    Manifold::unhalt();
    // LinkOutput::Send() is inherited from LinkOutputBase; it calls the
    // ScheduleRxEvent() method.
    MyDataType1 d1(1234);
    linkOutput1->Send(d1);
   ...
   * Test SendTick()
   * isTimed = false; isHalf = false;
   * SendTick() specifies a tick latency which should be used instead of the
   * default latency. In this test case the dynamically given latency > default
   * latency.
   */
  void testSendTick_0() {
    Link<MyDataType1> *link1 = new Link<MyDataType1>();
  ...
  static CppUnit::Test *suite() {
    ...
    mySuite->addTest(new CppUnit::TestCaller<LinkOutputTest>(
        "testSend_0", &LinkOutputTest::testSend_0));
    mySuite->addTest(new CppUnit::TestCaller<LinkOutputTest>(
        "testSend_1", &LinkOutputTest::testSend_1));
    mySuite->addTest(new CppUnit::TestCaller<LinkOutputTest>(
        "testSend_2", &LinkOutputTest::testSend_2));
    mySuite->addTest(new CppUnit::TestCaller<LinkOutputTest>(
        "testSend_3", &LinkOutputTest::testSend_3));

    mySuite->addTest(new CppUnit::TestCaller<LinkOutputTest>(
        "testSendTick_0", &LinkOutputTest::testSendTick_0));
    mySuite->addTest(new CppUnit::TestCaller<LinkOutputTest>(
        "testSendTick_1", &LinkOutputTest::testSendTick_1));
    return mySuite;
  }
};
