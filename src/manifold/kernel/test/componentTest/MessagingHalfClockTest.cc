...
//!
//! In this program, the components are clock based. The clock used is one other
//! than the master clock. The delay is specified in half ticks. This program
//! uses a template class, so it can be used to test all primitive types that
//! are sent between two components.
//!
#include <Test.h>
...
class MyObj1 : public Component {
 ...
 public:
...
#ifdef DBG
  MyObj1(const ofstream &dbg_log, const Clock &c)
      : m_clock(const_cast<Clock &>(c)),
  ...
  void handler(T data) {
    ...
    m_dbg_log << "@@@ " << Manifold ::NowTicks(m_clock) << " (half tick " << now
              << ")" << endl;
#endif
...
#ifdef DBG
    m_dbg_log << "@@@ " << Manifold ::NowTicks(m_clock) << " (half tick "
              << Manifold ::NowHalfTicks(m_clock) << ")" << endl;
    m_dbg_log << "    process_incoming  "
              << " inputIdex= " << inputIndex << " data= " << data << endl;
#endif
...
class MessagingTest : public CppUnit::TestFixture {
 ...
 public:
  ...
  void test_messaging() {
...
#ifdef DBG
    CompId_t c0 = Component ::Create<MyObj1<T> >(
        0, DBG_LOG, Clock1);  // c0 created for LP 0
    CompId_t c1 = Component ::Create<MyObj1<T> >(
        1, DBG_LOG, Clock1);  // c1 created for LP 1
#else
    CompId_t c0 =
        Component ::Create<MyObj1<T> >(0, Clock1);  // c0 created for LP 0
    CompId_t c1 =
        Component ::Create<MyObj1<T> >(1, Clock1);  // c1 created for LP 1
#endif
    ...
    // connect the two
    Manifold ::ConnectClockHalf(c0, MyObj1<T>::C0_OUT, c1, MyObj1<T>::C1_IN,
                                Clock1, &MyObj1<T>::process_incoming,
                                MyObj1<T>::C0_OUT_LATENCY);
    ...
    for (int i = 0; i < SIZE; i++) {
      ...
      Manifold ::ScheduleClockHalf(when[i], Clock1, &MyObj1<T>::handler, comp,
                                   send_data[i]);
    }
    const unsigned int STOP = 1000;
    CPPUNIT_ASSERT(when[SIZE - 1] < STOP);

    Ticks_t ScheduleAt = Manifold ::NowHalfTicks(
        Clock1);  // Tick time when we schedule the events.
    ...
    if (Mytid == 0) {
      ...
      MPI_Status status;
      MPI_Recv(recv_data, SIZE, MPI_UNSIGNED_LONG_LONG, 1, 0, MPI_COMM_WORLD,
               &status);
      ...
      for (int i = 0; i < SIZE; i++) {
        CPPUNIT_ASSERT_EQUAL(send_data[i],
                             (T)(recv_data[i]));  // cast back to T
        CPPUNIT_ASSERT_EQUAL(ScheduleAt + when[i] + MyObj1<T>::C0_OUT_LATENCY,
                             ticks[i]);
      }
    }
    else {
  ...
  static CppUnit::Test *suite() {
    ...
    mySuite->addTest(new CppUnit::TestCaller<MessagingTest>(
        "test_messaging_u32", &MessagingTest::test_messaging<uint32_t>));
    mySuite->addTest(new CppUnit::TestCaller<MessagingTest>(
        "test_messaging_u64", &MessagingTest::test_messaging<uint64_t>));
