...
//! tests non-primitive types that are sent between the two components. The data
//! type must implement Serialize() and Deserialize(). In this program data is
//! passed by value.
//!
#include <Test.h>
 ...
 public:
  ...
  void handler(MyPkt data) {
    ...
    if (Mytid == 0) {  // c0 sends messages to c1
      ...
      m_dbg_log << "    sendHalfTick= " << now
                << " recvHalfTick= " << now + C0_OUT_LATENCY << " data= " << hex
                << data << dec << endl;
#endif
    }
  }
  ...
  void process_incoming(int inputIndex, MyPkt data) {
    ...
    m_dbg_log << "    process_incoming  "
              << " inputIdex= " << inputIndex << " data= " << hex << data << dec
              << endl;
#endif
...
class MessagingTest : public CppUnit::TestFixture {
 ...
 public:
  ...
  void test_messaging() {
...
#ifdef DBG
    CompId_t c0 =
        Component ::Create<MyObj1>(0, DBG_LOG, Clock1);  // c0 created for LP 0
    CompId_t c1 =
        Component ::Create<MyObj1>(1, DBG_LOG, Clock1);  // c1 created for LP 1
#else
    ...
    // connect the two
    Manifold ::ConnectClockHalf(c0, MyObj1::C0_OUT, c1, MyObj1::C1_IN, Clock1,
                                &MyObj1::process_incoming,
                                MyObj1::C0_OUT_LATENCY);
    ...
    for (int i = 0; i < SIZE; i++) {
      ...
      Manifold ::ScheduleClockHalf(when[i], Clock1, &MyObj1::handler, comp,
                                   send_data[i]);
    }
