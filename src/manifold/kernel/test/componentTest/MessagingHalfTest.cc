...
//! master clock. The delay is specified in half ticks. This program uses a
//! template class, so it can be used to test all primitive types that are sent
...
class MyObj1 : public Component {
 ...
 public:
  ...
  void handler(T data) {
    ...
    m_dbg_log << "@@@ " << Manifold ::NowTicks() << " (half tick " << now << ")"
              << endl;
#endif
...
#ifdef DBG
    m_dbg_log << "@@@ " << Manifold ::NowTicks() << " (half tick "
              << Manifold ::NowHalfTicks() << ")" << endl;
...
class MessagingTest : public CppUnit::TestFixture {
 ...
 public:
  ...
  void test_messaging() {
...
#ifdef DBG
    CompId_t c0 =
        Component ::Create<MyObj1<T> >(0, DBG_LOG);  // c0 created for LP 0
    CompId_t c1 =
        Component ::Create<MyObj1<T> >(1, DBG_LOG);  // c1 created for LP 1
#else
    ...
    // connect the two
    Manifold ::ConnectHalf(c0, MyObj1<T>::C0_OUT, c1, MyObj1<T>::C1_IN,
    ...
    Ticks_t ScheduleAt =
        Manifold ::NowHalfTicks();  // Tick time when we schedule the events.
