...
//! than the master clock. This program uses a template class, so it can be used
//! to test all primitive types that are sent between two components.
//!
#include <Test.h>
  ...
  void handler(T data) {
    ...
    if (Mytid == 0) {  // c0 sends messages to c1
      ...
      m_dbg_log << "    sendTick= " << now
                << " recvTick= " << now + C0_OUT_LATENCY << " data= " << data
                << endl;
#endif
    }
  }
...
class MessagingTest : public CppUnit::TestFixture {
 ...
 public:
  ...
  void test_messaging() {
    ...
    // connect the two
    Manifold ::ConnectClock(c0, MyObj1<T>::C0_OUT, c1, MyObj1<T>::C1_IN, Clock1,
                            &MyObj1<T>::process_incoming,
    ...
    for (int i = 0; i < SIZE; i++) {
      ...
      Manifold ::ScheduleClock(when[i], Clock1, &MyObj1<T>::handler, comp,
                               send_data[i]);
    }
    ...
    Ticks_t ScheduleAt =
        Manifold ::NowTicks(Clock1);  // Tick time when we schedule the events.
