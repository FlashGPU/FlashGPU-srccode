...
//! This program uses a template class, so it can be used to test all primitive
//! types that are sent between two components.
//!
#include <Test.h>
  ...
  void handler(T data) {
    ...
    if (Mytid == 0) {  // c0 sends messages to c1
      ...
      m_dbg_log << "    sendTime= " << now
                << " recvTime= " << now + C0_OUT_LATENCY << " data= " << data
                << endl;
#endif
    }
  }
...
class MessagingTest : public CppUnit::TestFixture {
 ...
 public:
  ...
  void test_messaging() {
    ...
    // connect the two
    Manifold ::ConnectTime(c0, MyObj1<T>::C0_OUT, c1, MyObj1<T>::C1_IN,
    ...
    if (Mytid == 0) {
      ...
      for (int i = 0; i < SIZE; i++) {
        ...
        CPPUNIT_ASSERT_DOUBLES_EQUAL(
            ScheduleAt + when[i] + MyObj1<T>::C0_OUT_LATENCY, times[i],
            DOUBLE_COMP_DELTA);
      }
    }
    else {
