...
class ManifoldTest : public CppUnit::TestFixture {
 ...
 public:
  ...
  void testIsSafeToProcess() {
    ...
    when[0] = (Ticks_t)(random() / (RAND_MAX + 1.0) * 10 +
    ...
    for (unsigned int i = 0; i < calledTicks.size(); i++) {
      DBG_LOG << "calledTick= " << calledTicks[i]
              << ", called= " << (double)calledTicks[i] / MASTER_CLOCK_HZ
              << ", granted= " << grantedTimes[i] << endl;
      CPPUNIT_ASSERT((double)calledTicks[i] / MASTER_CLOCK_HZ <=
                     grantedTimes[i]);
    }
  }
