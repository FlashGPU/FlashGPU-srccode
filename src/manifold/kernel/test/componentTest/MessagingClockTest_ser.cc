...
//! than the master clock. This program tests non-primitive types that are sent
//! between the two components. The data type must implement Serialize() and
//! Deserialize(). In this program data is passed by value.
//!
#include <Test.h>
...
class MessagingTest : public CppUnit::TestFixture {
 ...
 public:
  ...
  void test_messaging() {
    ...
    // connect the two
    Manifold ::ConnectClock(c0, MyObj1::C0_OUT, c1, MyObj1::C1_IN, Clock1,
    ...
    for (int i = 0; i < SIZE; i++) {
      ...
      Manifold ::ScheduleClock(when[i], Clock1, &MyObj1::handler, comp,
                               send_data[i]);
    }
