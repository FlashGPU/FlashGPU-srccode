...
//! master clock. This program uses a template class, so it can be used to test
//! all primitive types that are sent between two components.
//!
#include <Test.h>
...
class MessagingTest : public CppUnit::TestFixture {
 ...
 public:
  ...
  void test_messaging() {
    ...
    // connect the two
    Manifold ::Connect(c0, MyObj1<T>::C0_OUT, c1, MyObj1<T>::C1_IN,
                       &MyObj1<T>::process_incoming, MyObj1<T>::C0_OUT_LATENCY);
  ...
  static CppUnit::Test *suite() {
    ...
    mySuite->addTest(new CppUnit::TestCaller<MessagingTest>(
        "test_messaging_uint32", &MessagingTest::test_messaging<uint32_t>));
    mySuite->addTest(new CppUnit::TestCaller<MessagingTest>(
        "test_messaging_uint64", &MessagingTest::test_messaging<uint64_t>));
