...
//! This program tests non-primitive types that are sent between the two
//! components. The data type must implement Serialize() and Deserialize(). In
//! this program data is passed by pointer.
//!
#include <Test.h>
  ...
  void handler(MyPkt *data) {
    ...
    if (Mytid == 0) {  // c0 sends messages to c1
      ...
      m_dbg_log << "    sendTime= " << now
                << " recvTime= " << now + C0_OUT_LATENCY << " data= " << hex
                << *data << endl;
#endif
    }
  }
...
class MessagingTest : public CppUnit::TestFixture {
 ...
 public:
  ...
  void test_messaging() {
    ...
    // connect the two
    Manifold ::ConnectTime(c0, MyObj1::C0_OUT, c1, MyObj1::C1_IN,
    ...
    if (Mytid == 0) {
      ...
      for (int i = 0; i < SIZE; i++) {
        ...
        CPPUNIT_ASSERT_DOUBLES_EQUAL(
            ScheduleAt + when[i] + MyObj1::C0_OUT_LATENCY, times[i],
            DOUBLE_COMP_DELTA);
      }
    }
    else {
