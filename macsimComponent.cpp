...
macsimComponent::macsimComponent(ComponentId_t id, Params &params)
  ...
  if (m_cube_connected) {
    m_cube_link = dynamic_cast<Interfaces::SimpleMem *>(
        loadModuleWithComponent("memHierarchy.memInterface", this, params));
    if (!m_cube_link)
      m_dbg->fatal(CALL_INFO, -1, "Unable to load Module as memory\n");
    m_cube_link->initialize("cube_link",
  ...
  registerClock(m_clock_freq, new Clock::Handler<macsimComponent>(
                                  this, &macsimComponent::ticReceived));
  ...
  // When SLAVE mode, MacSim awaits trigger event to arrive, which will cause
  // MacSim to begin execution of a specified kernel.
...
void macsimComponent::configureLinks(SST::Params &params) {
  for (unsigned int l = 0; l < m_num_link; ++l) {
    Interfaces::SimpleMem *icache_link = dynamic_cast<Interfaces::SimpleMem *>(
        loadModuleWithComponent("memHierarchy.memInterface", this, params));
    if (!icache_link)
      m_dbg->fatal(CALL_INFO, -1, "Unable to load Module as memory\n");
    Interfaces::SimpleMem *dcache_link = dynamic_cast<Interfaces::SimpleMem *>(
        loadModuleWithComponent("memHierarchy.memInterface", this, params));
    if (!dcache_link)
      m_dbg->fatal(CALL_INFO, -1, "Unable to load Module as memory\n");
    ...
    icache_link->initialize(
        icache_link_name.str(),
        new Interfaces::SimpleMem::Handler<macsimComponent>(
    ...
    dcache_link->initialize(dcache_link_name.str(),
                            new Interfaces::SimpleMem::Handler<macsimComponent>(
    ...
    if (m_ptx_core) {
      Interfaces::SimpleMem *ccache_link =
          dynamic_cast<Interfaces::SimpleMem *>(loadModuleWithComponent(
              "memHierarchy.memInterface", this, params));
      if (!ccache_link)
        m_dbg->fatal(CALL_INFO, -1, "Unable to load Module as memory\n");
      Interfaces::SimpleMem *tcache_link =
          dynamic_cast<Interfaces::SimpleMem *>(loadModuleWithComponent(
              "memHierarchy.memInterface", this, params));
      if (!tcache_link)
        m_dbg->fatal(CALL_INFO, -1, "Unable to load Module as memory\n");
      ...
      ccache_link->initialize(
          ccache_link_name.str(),
          new Interfaces::SimpleMem::Handler<macsimComponent>(
      ...
      tcache_link->initialize(
          tcache_link_name.str(),
          new Interfaces::SimpleMem::Handler<macsimComponent>(
...
void macsimComponent::init(unsigned int phase) {
  if (!phase) {
    for (unsigned int l = 0; l < m_num_link; ++l) {
      m_instruction_cache_links[l]->sendInitData(
          new Interfaces::StringEvent("SST::MemHierarchy::MemEvent"));
      m_data_cache_links[l]->sendInitData(
          new Interfaces::StringEvent("SST::MemHierarchy::MemEvent"));
    }

    if (m_cube_connected)
      m_cube_link->sendInitData(
          new Interfaces::StringEvent("SST::MemHierarchy::MemEvent"));
  }
}
...
void macsimComponent::setup() {
  ...
  for (auto I = tokens.begin(), E = tokens.end(); I != E; ++I) {
    string command = *I;
    argv[argc] = new char[command.size() + 1];
    strcpy(argv[argc], command.c_str());
    argc++;
  }
...
bool macsimComponent::ticReceived(Cycle_t) {
  ...
  if (m_cycle % 100000 == 0) {
    for (unsigned int l = 0; l < m_num_link; ++l) {
      if (m_ptx_core) {
        MSC_DEBUG("Core[%2d] I$: (%lu, %lu), D$: (%lu, %lu) C$: (%lu, %lu), "
                  "T$: (%lu, %lu)\n",
                  ...
                  m_const_cache_request_counters[l],
